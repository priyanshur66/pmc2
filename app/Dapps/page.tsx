...
interface TokenCardProps {
  ...
  const TokenCard: React.FC<TokenCardProps> = ({ name, symbol, price, change, iconSrc, changePositive }) => {
    return (
      <div className="bg-gray-800 rounded-lg p-4 flex justify-between items-center mb-4 shadow-lg">
        <div className="flex items-center">
          <img src={iconSrc} alt={`${name} icon`} className="w-10 h-10 rounded-full mr-4" />
          <div>
            <h2 className="text-lg font-semibold">{name}</h2>
            <p className="text-sm text-gray-400">{price} <span className={changePositive ? 'text-green-500' : 'text-red-500'}>{change}</span></p>
          </div>
        </div>
...
<div className=" min-h-screen text-white p-4">
      ...
      <div className="flex space-x-4 mb-4">
        <button className="text-white border-b-2 border-red-500 pb-1">Swap</button>
      ...
      <div className="bg-gray-800 rounded-lg p-4 flex justify-between items-center">
        <div className="flex items-center">
          <img src="ziptos.svg" alt="DApp Icon" className="w-10 h-10 rounded-full mr-4" />
          <div>
